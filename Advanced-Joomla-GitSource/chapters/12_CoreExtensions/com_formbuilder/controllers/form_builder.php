...
class Form_builderControllerform_builder extends JControllerAdmin {
	...
	<tr>
		...
		<th width=5%> <?php echo JText::_( 'JSON?' ); ?> </th>
	...
	<?php
		...
		foreach ($rows as $row) {
			...
			$out .= "<tr>" .
				"<td><input id='cb{$i}' name='cid[]' onclick='Joomla.isChecked(this.checked);' type='checkbox' /></td>" .
			...
			$archived = false;
			$trashed = false;
			$item = array('state' => 1, 'publish_up' => '0000-00-00 00:00:00', 'publish_down' => '0000-00-00 00:00:00', 'name' => 'HELLOW');
			$out .= JHtml::_('jgrid.published', $item['state'], $i, 'banners.', $canChange, 'cb', $item['publish_up'], $item['publish_down']);
			// Create dropdown items
			$action = $archived ? 'unarchive' : 'archive';
			JHtml::_('actionsdropdown.' . $action, 'cb' . $i, 'formbuilder');

			$action = $trashed ? 'untrash' : 'trash';
			JHtml::_('actionsdropdown.' . $action, 'cb' . $i, 'formbuilder');

								// Render dropdown list
			$out .= JHtml::_('actionsdropdown.render', $db->escape($item['name']));
				$out .= '			</div>
						</td>';
